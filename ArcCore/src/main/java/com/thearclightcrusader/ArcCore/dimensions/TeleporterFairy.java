...
public class TeleporterFairy extends Teleporter
{
    ...
    public TeleporterFairy(WorldServer par1WorldServer)
    {
    ...
    public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
    {
        ...
        else
        {
            for (k1 = l - short1; k1 <= l + short1; ++k1)
            {
                ...
                for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
                {
                    ...
                    {
                        if (this.worldServerInstance.getBlock(k1, i2, l1) == Main.fairyPortal)
                        {
                            while (this.worldServerInstance.getBlock(k1, i2 - 1, l1) == Main.fairyPortal)
                            {
                                --i2;
                            }
        ...
        if (d3 >= 0.0D)
        {
            ...
            if (this.worldServerInstance.getBlock(i - 1, j, k) == Main.fairyPortal)
            {
                j2 = 2;
            }
            ...
            if (this.worldServerInstance.getBlock(i, j, k - 1) == Main.fairyPortal)
            {
                j2 = 3;
            }
            ...
            for (k2 = -1; k2 <= 1; ++k2)
            {
                for (i3 = 1; i3 < 3; ++i3)
                {
                    for (l2 = -1; l2 < 3; ++l2)
                    {
                        ...
                        i4 = j2 + (i3 - 1) * l5 - k2 * k5;
                        flag = l2 < 0;
                        this.worldServerInstance.setBlock(k3, j3, i4, flag ? Main.fairyRitualStone : Blocks.air);
                    }
                }
            }
        }
        ...
        for (k2 = 0; k2 < 4; ++k2)
        {
            for (i3 = 0; i3 < 4; ++i3)
            {
                for (l2 = -1; l2 < 4; ++l2)
                {
                    ...
                    this.worldServerInstance.setBlock(k3, j3, i4, flag ? Main.fairyRitualStone : Main.fairyPortal, 0, 2);
                }
            }
